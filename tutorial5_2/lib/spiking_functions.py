...
# Student name : Joonsu Gha
# Student ID   : 21-980-958
# Email address: joogha@student.ethz.ch
...
def derivative_hard_sigmoid(x):
    ...
    """
    deriv = torch.heaviside(x, torch.tensor([1.])) - torch.heaviside(x - 1, torch.tensor([1.]))
    return deriv
...
def accuracy_on_spikes(S, T):
    ...
    acc = torch.sum(torch.count_nonzero(Y-T)) / T.shape[0]
    return acc
...
def spike_regularizer(spikes_list):
    ...
    """
    L1_loss, L2_loss = 0., 0.
    for activity in spikes_list:
        L1_loss += activity.sum()
        L2_loss += activity.sum((0, 1)).square().sum() / activity.shape[2]
